...
def dataloader(config):
    ...
    files = os.listdir(path_to_data) # each file contains 600 data samples
    ...
    if preload:
        ...
        for f in tqdm(files, disable=tqdm_disable):
            ...
            if file_id in file_indexes:
                ...
                for i in range(len(horizontal_radar_map)):
        ...
        joints = torch.tensor(joints_list)  # shape: (num_samples, 14, 2)
        ...
        for f in tqdm(files, disable=tqdm_disable):
            ...
            if file_id in file_indexes:
                data = pickle.load(open(path_to_data + f, 'rb'))
                horizontal_radar_map = data['hori']
                for i in range(len(horizontal_radar_map)):
                    data_paths['file_path'].append(path_to_data + f)
                    data_paths['index'].append(i)
        return data_paths
...
class HuPR_Dataset(Dataset):
    def __init__(self,config):
        ...
        if self.preload:
            ...
            self.image_path = []
            self.data_paths = dataloader(config)
            self.num_samples = len(self.data_paths['file_path'])
            print("The number of samples in the dataset: ", self.num_samples)

    def __len__(self):
        if self.preload:
            return len(self.radar_maps)
        else:
    ...
    def __getitem__(self, idx):
        if self.preload:
            ...
            radar_map, joints, bbox, path_to_rgb_image = self._load_data(idx)
            self.image_path.append(path_to_rgb_image)
            return radar_map,joints, bbox
           
    
    def _load_data(self, idx):
        data = pickle.load(open(self.data_paths['file_path'][idx], 'rb'))
        horizontal_radar_map = data['hori']   # List of maps with shape: (64, 64, 8) (range_bin, azimuth_bin, elevation_bin), complex values
        vertical_radar_map = data['vert']     # List of maps with shape: (64, 64, 8), complex values
        labels = data['labels']
        rgb_image_folder = data['rgb_image_folder']
        i = self.data_paths['index'][idx]
        h_map = horizontal_radar_map[i]      # complex values, shape: (64, 64, 8)
        v_map = vertical_radar_map[i]        # complex values, shape: (64, 64, 8)
        label = labels[i]           # dictionary
        joints = label['joints']          # shape: (14, 2)
        joints = torch.tensor(joints)
        bbox = label['bbox']       # shape: (4,)
        bbox = torch.tensor(bbox)
        path_to_rgb_image = self.config['dataset_path'] + '/' + rgb_image_folder + '/%09d.jpg'%(int(i))
        radar_map = np.stack((h_map, v_map), axis=-1)     # shape: (64, 64, 8, 2)
        radar_map = torch.tensor(radar_map, dtype=torch.complex64)
        if self.config['format'] == 'polar':
            radar_map_amp = torch.abs(radar_map)
            radar_map = torch.stack((radar_map_amp, torch.angle(radar_map)), dim=-1)  # shape: (64, 64, 8, 2, 2)
        elif self.config['format'] == 'cartesian':
            radar_map = torch.stack((radar_map.real, radar_map.imag), dim=-1)    # shape: (64, 64, 8, 2, 2)
        elif self.config['format'] == 'complex':
            pass
        else:
            raise ValueError('Invalid format')
        return radar_map, joints, bbox, path_to_rgb_image
